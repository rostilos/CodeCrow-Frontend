...
function normalizeProvider(provider: string): VcsProvider {
  ...
  return (
    mapping[provider] ||
    (provider.toLowerCase().replace("_", "-") as VcsProvider)
  );
}
...
class IntegrationService extends ApiService {
  ...
  async getConnections(
    ...
    const connections = await this.request<VcsConnection[]>(url, {
      method: "GET",
    });
  ...
  async refreshConnectionToken(
    ...
    const connection = await this.request<VcsConnection>(
      `/${workspaceSlug}/integrations/${provider}/connections/${connectionId}/refresh-token`,
      { method: "POST" },
    );
    return normalizeConnection(connection);
  }
  ...
  async onboardRepositories(
    ...
    for (const { externalRepoId, request } of repositories) {
      try {
        const result = await this.onboardRepository(
          workspaceSlug,
          provider,
          externalRepoId,
          request,
   ...
   */
  async startConnectAppInstall(
    workspaceId: number,
    workspaceSlug?: string,
  ): Promise<ConnectInstallStartResponse> {
  ...
  async getBitbucketConnectStatus(): Promise<{ configured: boolean }> {
    return this.request<{ configured: boolean }>("/bitbucket/connect/status", {
      method: "GET",
    });
  }
   ...
   */
  async getUnlinkedConnectInstallations(): Promise<
   ...
   */
  async getConnectInstallationsForWorkspace(
    workspaceId: number,
  ): Promise<BitbucketConnectInstallation[]> {
    return this.request<BitbucketConnectInstallation[]>(
   ...
   */
  async checkConnectInstallStatus(
    state: string,
